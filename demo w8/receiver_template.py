    ...
    def run(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                seq_no = int.from_bytes(incoming_message[:2], byteorder='big')
                packet_type = int.from_bytes(incoming_message[2:4], byteorder='big')
                if packet_type == 2: #  SYN
                    logging.debug("it a SYN packet!!!")
                elif packet_type == 1:
                    logging.debug("it is a DATA packet")

                logging.debug(f"seq = {seq_no}")
                logging.debug(f"Get a new message: {incoming_message}")
            except ConnectionResetError:
                continue

            # randomly drop the packet
            if random.randint(1,100) > 50: # 90% of chance
                logging.debug("drop a packet!")
                continue

            logging.debug(f"client{sender_address} send a message: len= {len(incoming_message.decode('utf-8'))}")
            # with open(self.filename, "a+") as file:
            #     file.write(incoming_message.decode())

            # reply "ACK" once receive any message from sender
            reply_message = f"ACK for packet: x"
