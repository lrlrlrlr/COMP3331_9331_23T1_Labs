    ...
    def ptp_open_and_send(self):
        ...
        with open("random1.txt", "rb") as file:
            ...
            while True:
                ...
                if content:
                    self.seq_dict[i] = time.time()

                    seq_no = random.randint(1, 2^16-1)
                    packet_type = 1 # SYN
                    payload = b""
                    header = seq_no.to_bytes(2, "big") + packet_type.to_bytes(2, "big")
                    content = header + payload
                    # content = seq_no.to_bytes(2, "big") + content
                    ...
                    logging.debug(f"sent {i + 1} packet, done.")
                    break
